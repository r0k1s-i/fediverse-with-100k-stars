...
var dustTexture = textureLoader.load(
  "src/assets/textures/dust.png",
  undefined,
  undefined,
  onTextureError,
);
...
export function generateDust() {
  ...
  for (var i = 0; i < count; i++) {
    ...
    var thickness =
      constrain(
        Math.pow(constrain(60 - distanceToCenter * 0.1, 0, 100000), 2) * 0.02,
        2,
        10000,
      ) +
      Math.random() * 20;
    ...
    if (Math.random() > 0.99)
      size *= Math.pow(1 + Math.random(), 4 + Math.random() * 3) * 0.2;
  ...
  pDustSystem.update = function () {
    ...
    if (camera.position.z < VISIBILITY.GALAXY.HIDE_Z) {
      if (dustShaderMaterial.opacity > 0) dustShaderMaterial.opacity -= 0.05;
    } else {
      if (dustShaderMaterial.opacity < 1) dustShaderMaterial.opacity += 0.05;
    }
